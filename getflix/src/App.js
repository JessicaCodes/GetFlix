...
updateState(title,selected){
  this.setState({ linkTitle: title, linkSelected: selected})
}
  ...
  checkForSelectedLink(){
    ...
    const mainTitles = (
     ...
     const selectedLink = (
       <MovieCategory title={this.state.linkTitle} movies={this.state.linkSelected}/>
     )

     return this.state.searchList.length > 0 ? mainTitles : selectedLink;
    }
